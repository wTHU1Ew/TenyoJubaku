...
func TestDynamicSLTracker_Validate(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			wantErr: true,
			errMsg:  "initial_sl_price must be positive",
		},
		{
			name: "leverage less than 1",
			tracker: &DynamicSLTracker{
				PositionKey:    "BTC-USDT-SWAP_long",
				InstId:         "BTC-USDT-SWAP",
				PosSide:        "long",
			...
			wantErr: true,
			errMsg:  "leverage must be at least 1",
		},
		{
			name: "negative move_count",
			tracker: &DynamicSLTracker{
				PositionKey:    "BTC-USDT-SWAP_long",
				InstId:         "BTC-USDT-SWAP",
				PosSide:        "long",
			...
			wantErr: true,
			errMsg:  "move_count cannot be negative",
		},
	}
